...
class CLSDataset(Dataset):
    ...
    def _preprocess_features(self):
        ...
        for class_name in tqdm.tqdm(self.class_list):
            ...
            text_features = extract_sentence_features(prompt)
            text_features_tensor = torch.tensor(text_features, dtype=torch.float32).unsqueeze(0).to(self.device)
            
            # Process with lens model
            with torch.no_grad():
                h_pos, h_neg = self.lens_model(text_features_tensor)
            # Store positive and negative features
            self.pos_features.append(h_pos.cpu().numpy()[0])
            self.neg_features.append(h_neg.cpu().numpy()[0])
        ...
        torch.save({
            ...
            'labels': self.labels,
            'pos_features': self.pos_features, # [num_classes, embed_dim]
            'neg_features': self.neg_features # [num_classes, embed_dim]
        }, cache_path)
    ...
    def __getitem__(self, idx):
        ...
        Returns:
            ...
            pos_features: List of positive features for each option [num_classes, embed_dim]
            neg_features: List of negative features for each option [num_classes, embed_dim]
        ...
        pos_features = torch.tensor(np.stack(self.pos_features), dtype=torch.float32) # [num_classes, embed_dim]
        neg_features = torch.tensor(np.stack(self.neg_features), dtype=torch.float32)
...
def test_clip_glasses_frame(cfg):
    ...
    test_dataset = CLSDataset(cfg.test_csv_path, Clip_model, lens_model)
    ...
    with torch.no_grad():
        for img_features, text_features, _,  labels in tqdm.tqdm(test_loader, desc="Validation"):
            ...
            if cfg.raw_CLIP:
                # ============raw=================  
                # 用 原生Clip_model 计算相似度
                logit_scale = Clip_model.logit_scale.exp() # 温度参数 τ 的倒数

                text_feats = F.normalize(text_features[0], dim=1) # [num_classes, embed_dim]
                img_feats = F.normalize(img_features, dim=1) # [num_images=batch_size, embed_dim]
                all_scores = logit_scale * img_feats @ text_feats.t()  # [num_images=batch_size, num_classes]
            else:
                # ===========ours==============
                text_feats = text_features[0] # [num_classes, embed_dim]
                all_scores, _ = frame_model(img_features, text_feats) # [num_classes, num_images=batch_size]
                all_scores = all_scores.t()  # [num_images=batch_size, num_classes]
                # ============================= 
...
def main():
    ...
    cfg = {
        ...
        # -----常规参数-----
        'csv_path': '/root/NP-CLIP/NegBench/data/CLS_Imagenet/imagenet_train.csv',  # Path to training CSV file
