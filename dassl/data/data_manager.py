...
class DataManager:
    ...
    def __init__(self, cfg, custom_tfm_train=None, custom_tfm_test=None, dataset_transform=None):
        ...
        if dataset.val:  # 构建验证数据加载器 val_loader (如果存在验证数据)
            val_sampler = build_test_sampler(dataset.val) # 构建验证集采样器
        ...
        test_sampler = build_test_sampler(dataset.test) # 构建测试集采样器
...
class TransformeWrapper(TorchDataset):
    ...
    主要功能：
        - 对数据源中的每个数据项应用数据转换 (resize + toTensor + normalize)。
    ...
    def __init__(self, cfg, data_source, transform=None, is_train=False):
        ...
        主要步骤：
            1. 初始化属性，并获取相关配置信息。
            3. 构建一个不应用任何数据增强的增强方式 self.to_tensor。
        """
        # 初始化属性
    ...
    def __getitem__(self, idx):
        ...
        返回：字典 output: 
            ...
            - img0: 未增强处理的图像 | 即只经过预处理 (resize+toToTensor+normalize) 的图像
        """
        ...
        if self.transform is not None:  
            ...
            output["img"] = image_to_tensor_pipeline(img0)  # 原始图像

        if self.return_img0:  # 如果需要返回未增强的原始图像
            output["img0"] = image_to_tensor_pipeline(img0) # 只经过预处理管道处理未进行增强的图像
